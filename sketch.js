...
var polygroup;
var menu;
var ballTrowerButton,colorMatcherButton,oddOneOutButton;
var ballTrower_icon,colorMatcher_icon,oddOneOutButton_icon;
...
ballTrower_icon = loadImage("balltrower icon.png");
colorMatcher_icon = loadImage("colormatcher icon.png");
oddOneOutButton_icon = loadImage("oddoneout icon.png");
...
menu = createSprite(400,50,400,45);
menu.shapeColor=255;

fill("white");
ballTrowerButton=createSprite(250,50,25,25);
ballTrowerButton.addImage(colorMatcher_icon);
ballTrowerButton.scale=0.15;

colorMatcherButton=createSprite(400,50,25,25);
colorMatcherButton.addImage(oddOneOutButton_icon);
colorMatcherButton.scale=0.25;

oddOneOutButton=createSprite(550,50,25,25);
oddOneOutButton.addImage(ballTrower_icon);
oddOneOutButton.scale=0.25;
...
function draw() {
  ...
  if(mousePressedOver(ballTrowerButton)){
    PUZZLECHOOSER = 2;
    lever.destroy();	
    hiderbackground.destroy();
    
     }
     if(mousePressedOver(colorMatcherButton)){
      PUZZLECHOOSER = 3;
...
line(lever.x,0,lever.x,700);	
 ...
 edge4.display();
 fill(255);
 text(250,200,"color matcher");
 text(400,200,"odd one out button");
 text(550,200,"ball trower");
//g 
}
